...
init
{
	if (game.ProcessName == "Project64") {
		...
		var peHdrAddr = baseAddr + memory.ReadValue<int>(baseAddr + 0x3C);
		var textSectSize = memory.ReadValue<int>(peHdrAddr + 0x100);
		var textSectStart = baseAddr + memory.ReadValue<int>(peHdrAddr + 0x104);
		var idataSectStart = baseAddr + memory.ReadValue<int>(peHdrAddr + 0x17C);
		var isLoadingAddr = idataSectStart - 4;

		var scanner = new SignatureScanner(game, textSectStart, textSectSize);
